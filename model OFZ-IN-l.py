...
const = get_constants()
# Подготовка инфляции и ставки депозита
...
# %%
def calculate_ofz_in_l(inf_res, const):
    ...
    'Доход с вычетом']]
    return ofz_in_l
ofz_in_l = calculate_ofz_in_l()
# %% [markdown]
# # 4. ОФЗ ПД

# %%
def calculate_ofz_pd(ofz_in_l,const):
...
# %%
def calculate_depozit(ofz_in_l,inf_res,const):
...
depozit = calculate_depozit()
# %% [markdown]
# # Итоговая таблица доходов 

# %%
def build_summary_table (ofz_in_l, ofz_pd, depozit, inf_res):
...
# %%
def build_government_burden( ofz_pd, inf_res, const):
# ОФЗ ИН (л)
    ...
    total_ofz_pd_gos = pd.DataFrame(total_ofz_pd_gos[cols2].sum()).T
    total_ofz_pd_gos['Год'] = 'Итого'
    itog_ofz_pd = pd.concat([ofz_pd_gos,total_ofz_pd_gos],ignore_index=True)
    return itog_ofz_in_l_gos,itog_ofz_pd

# ============================================
# 8. Главная функция, запускающая всю модель
# ============================================
def run_model():
    inf_res, const = prepare_inflation_and_rate()
    ofz_in_l = calculate_ofz_in_l(inf_res, const)
    ofz_pd = calculate_ofz_pd(ofz_in_l, const)
    depozit = calculate_depozit(ofz_in_l,inf_res,const)
    doxod_za_period = build_summary_table(ofz_in_l, ofz_pd, depozit, inf_res, const)
    itog_ofz_in_l_gos,itog_ofz_pd = build_government_burden(ofz_in_l, ofz_pd, const)
